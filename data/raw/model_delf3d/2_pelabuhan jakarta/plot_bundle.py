...
def regridJakarta():
    ...
    nc_file = '/Users/maritim/Project/BMKG/SIMPEL/data/raw/model_delf3d/2_pelabuhan jakarta/bundle.nc'
    wave_height_tkl = '/Users/maritim/Project/BMKG/SIMPEL/data/raw/model_delf3d/2_pelabuhan jakarta/wave_height.tkl'
    ...
    for idt, time in enumerate(s1):
        for idx, x in enumerate(time):
            for idy, y in enumerate(x):
                ...
                arr_windu[idt, lat_near, lng_near] = windu[idt, idx, idy]
        ...
        masked_windu = ma.array(arr_windu[idt, :], mask=water_mask, fill_value=0).filled(0)
        masked_windv = ma.array(arr_windv[idt, :], mask=water_mask, fill_value=0).filled(0)
        ...
        img_winduv = coder.encode_uv_image(np.nan_to_num(masked_windu), np.nan_to_num(masked_windv), bound)
        ...
        if img_winduv:
            img_winduv.save(target_folder_wind + fname)
        if img_waveuv:
